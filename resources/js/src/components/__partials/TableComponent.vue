...
<template>
    <v-data-table class="elevation-1 ma-3 pb-3" :item-key="itemKey" :search="search" :headers="headerData"
        :items="result" :hide-default-footer="hideFooterAction" :loading="withLoading" :no-data-text="emptyDataText"
        ...
        <template v-slot:item.curricular="{ item }">
            <div class="d-grid my-2" v-for="curriculum in item.curricular" :key="curriculum.lesson_name">
                <v-chip color="info">
                    {{ curriculum.classroom_label }}
                </v-chip>

                <v-chip class="mx-3" color="success">
                    Maksimal Jam Per Minggu : {{ curriculum.maximum_hours_per_week }}
                </v-chip>
            </div>
        </template>
        ...
        <template v-slot:item.user_activation_status="{ item }">
            <v-chip small :color="item.user_activation_status === 'Aktif' ? 'success' : 'error'" dark>
        ...
        <template v-slot:item.actions="{ item }">
            <v-container class="d-flex justify-center">
                <v-btn v-if="withViewData" outlined v-on:click="$emit('view-data', item.id)" color="info">
                    <v-icon left>
                        mdi-eye
                    </v-icon>
                    Lihat
                </v-btn>
                <v-btn outlined v-on:click="$emit('edit-data', item.id)" color="primary" class="mx-3">
                    <v-icon left>
                        mdi-pencil
                    </v-icon>
                    Edit
                </v-btn>
                <v-btn outlined v-if="withDestroyData" v-on:click="$emit('destroy-data', item.id)" color="error">
                    <v-icon left>
                        mdi-delete
                    </v-icon>
                    Hapus
